<template>
  <el-dialog v-model="visible" :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="rules" ref="dataFormRef" @keyup.enter="dataFormSubmitHandle()" label-width="120px">
      ...
      <el-form-item prop="name" label="评论内容">
        <el-input v-model="dataForm.name" placeholder="评论内容"></el-input>
      </el-form-item>
      <el-form-item prop="parentName" label="选择评论" class="menu-list">
        <el-popover ref="menuListPopover" placement="bottom-start" trigger="click" :width="400" popper-class="popover-pop">
          <template v-slot:reference>
            <el-input v-model="dataForm.parentName" :readonly="true" placeholder="选择评论">
      ...
      <el-form-item v-if="dataForm.menuType === 0" prop="url" label="楼主">
        <el-input v-model="dataForm.url" placeholder="楼主"></el-input>
      </el-form-item>
      ...
      <el-form-item prop="permissions" label="用户">
        <el-input v-model="dataForm.permissions" placeholder="用户"></el-input>
      </el-form-item>
...
// 获取菜单列表
const getMenuList = () => {
  return baseService.get("/sys/pinglun/list?type=0").then((res) => {
...
const getInfo = (id: number) => {
  baseService.get(`/sys/pinglun/${id}`).then((res) => {
...
const dataFormSubmitHandle = () => {
  dataFormRef.value.validate((valid: boolean) => {
    ...
    (!dataForm.id ? baseService.post : baseService.put)("/sys/pinglun", dataForm).then((res) => {
      ElMessage.success({
        message: "成功",
        duration: 500,
        onClose: () => {
